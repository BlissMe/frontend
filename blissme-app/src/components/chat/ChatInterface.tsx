...
const ChatInterface = () => {
  ...
  const handleSend = async () => {
    ...
    const botReply = await chatBotService(context, inputValue, sessionSummaries, askedPhq9Ids);
  ...
  return (
    ...
    mx-auto md:mx-0 md:mr-12 mt-12 md:mt-0 backdrop-blur-sm"
      >
        {/* Chat Area */}
        <div className="flex-1 overflow-y-auto px-2 md:px-4 space-y-6">
          {messages.map((msg, index) => (
            ...
            >
              ...
              >
                {msg.sender === "you" ? (
                  <img src={user} alt="User" className="w-8 h-8 md:w-10 md:h-10 rounded-full" />
                ) : (
                  <img
              ...
              <Text className="text-xs text-gray-500 mt-1 ml-10 md:ml-12">{msg.time}</Text>
        ...
        <div className="flex items-center justify-between gap-2 mt-3">
          ...
          <Button
            type="text"
            icon={<img src={assets.send_icon} alt="send" className="w-6 h-6 md:w-8 md:h-8" />}
            onClick={handleSend}
            disabled={loading}
          />
        </div>
